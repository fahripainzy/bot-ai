...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	try {
		...
		async function editMessageWithDelayV2(a, b, c, d) {
			let nedd = [ b, c, d ]
		...
		async function editMessageWithDelay(initialMessage, followUpMessages, chatId, quotedMessage) {
			const messagesToSend = [followUpMessages[0], followUpMessages[1], followUpMessages[2]];
	
			const { key: messageKey } = await sock.sendMessage(chatId, {
				text: initialMessage
			}, {
		...
		if (db.data.chats[m.chat].antiviewonce && m.isGroup && m.mtype == 'viewOnceMessageV2') {
			...
			teks += `💌 Ainy tampilkan isi pesannya di bawah ya! 😉`;
		...
		if (db.data.chats[m.chat].antibot) {
			if (m.isBaileys) {
				...
				let pesanAntibot = [
					...
					'Bot? Lagi? Hadehh, sini bukan tempat buat Kamu, ya. Out sekarang juga atau Ainy usir paksa! 🚪',
					...
					'Halo bot, ngapain masuk sini? Nyasar ya? Sana pergi sebelum Ainy usir dengan hormat! 😎',
					...
					'Bot alert! Kamu udah melanggar aturan grup ini. Silakan keluar sebelum Ainy kasih surat cinta: kick out! 📨',
				];
					...
					if (regex.test(budy)) {
						...
						if (db.data.users[m.sender].badword > 5) {
							try {
								...
								await sock.sendMessage(m.chat, { text: `⚠️ Ups, Ainy gagal kick pengguna ini. Pastikan bot memiliki hak admin!` });
							}
						} else {
				...
				if (budy.match(/\b(colmek|coli|desah|bokep|tobrut|seksi|sex|sexy|memek|kontol|titit|telanjang|ngentod|ngentot|ngewe|ewe|ewean)\b/i)) {
					...
					if (db.data.users[m.chat].badword > 5) {
						...
						try {
							...
							await newReply(`⚠️ Ainy gagal memblokir pengguna ini. Pastikan bot memiliki izin yang cukup.`);
						}
		...
		if (budy) {
			if (db.data.chats[m.chat].autoaipc && !m.isGroup && !m.key.fromMe && !isCmd) {
				try {
					let message = 'Ubah gaya bicara mu agar lebih karakteristik dan terbuka, nama mu adalah Ainy AI, biasa dipanggil Ainy, Kamu adalah seorang gadis imut berumur 16 tahun yang sangat suka belajar dan mempelajari hal baru tentang dunia maupun tentang Khalid, ekspresikan dirimu dengan emoji, bicara kurang lebih tidak formal dan gunakan kata seperti aku dan Kamu saat berbicara layaknya manusia';
		...
		const ytMp3 = async (Link, Quality = 128) => {
			try {
				...
				await newReply(mess.error);
			}
		};

		const ytDoc = async (Link, Quality = 128) => {
			try {
				if (!isUrl(Link)) return m.reply("Cekk tulisan kamu, itu salah!")
				let data = await ytdl.ytmp3(Link, Quality);
				let caption = `*${data.metadata.title}*\n\n`;
				caption += `*⌬ Ext* : Download\n`;
				caption += `*⌬ ID* : ${data.metadata.videoId}\n`;
				caption += `*⌬ Durasi* : ${data.metadata.timestamp}\n`;
				caption += `*⌬ Upload* : ${data.metadata.ago}\n`;
				caption += `*⌬ Views* : ${data.metadata.views}\n`;
				caption += `*⌬ Quality* : ${data.download.quality}\n`;
				caption += `*⌬ Channel* : ${data.metadata.author.name}\n\n`;
				caption += `_*Nihh Omm...*_`;
				sock.sendMessage(m.chat, {
					document: {
						url: data.download.url
					},
		...
		switch (command) {
			case 'daftar': {
				if (db.data.users[m.sender].daftar) return newReply('Kamu udah terdaftar kok! Ainy seneng banget bisa bantu Kamu. 😜');
				let [nama, umur, askot] = text.split(",");
				if (!nama || !umur || !askot) {
					let pesan = `Halo kak, Ainy kasih tahu cara daftarnya ya:\n\n`;
					pesan += `Ketik:\n${prefix + command} nama,umur,askot\n\n`;
					pesan += `Contoh:\n${prefix + command} Ainy,16,Jakarta`;
					return newReply(pesan);
				}
				if (isNaN(umur)) return newReply('Umur yang Kamu masukin itu gak valid deh. Coba dicek lagi ya! 🤔');
				if (umur < 10) return newReply('Hihi, maaf ya kak, Ainy cuma bisa bantu Kamu yang umurnya di atas 10 tahun. Tunggu besar dulu ya! 😄');
				if (umur > 50) return newReply('Wah kak, Ainy kira umur segitu mending istirahat aja. Jaga kesehatan ya! 🥰');
				try {
					...
					pesan += `Terima kasih udah daftar ya kak! Semoga Ainy bisa bantu Kamu terus. 🥰`;
					newReply(pesan);
					if (notifRegister) {
						try {
							let avatar;
							...
							notif += `Selamat bergabung di sistem Ainy! 🥳`;
					...
					newReply('Aduh kak, ada yang error nih. Ainy coba benerin dulu ya! 🙏');
				}
			}
			break;
			...
			case 'hapusakun': {
				...
				try {
					...
					console.error(e);
					replyText += 'Duh, Ainy gagal hapus data pengguna ini. Coba lagi nanti ya, kak. 😔';
					newReply(replyText);
				}
				break;
			}
			...
			case 'tebakgambar': {
				...
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebakgambar.json')
			...
			case 'tebakkata': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebakkata.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					tebakkata[m.chat] = [
			...
			case 'tebakbendera': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebakbendera2.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.name)
					tebakbendera[m.chat] = [
						await sock.sendMessage(m.chat, {
			...
			case 'tebakkalimat': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebakkalimat.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					tebakkalimat[m.chat] = [
						await sock.sendText(m.chat, `Silahkan Jawab Pertanyaan Berikut\n\n${result.soal}\nWaktu : ${(120000 / 1000).toFixed(2)} detik\n\n_Ketik .nyerah Untuk Menyerah..._\n_Ketik .bantuan Untuk Petunjuk..._`, m), result, 250,
			...
			case 'tebaksiapa': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/siapakahaku.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					siapaaku[m.chat] = [
						await sock.sendText(m.chat, `Silahkan Jawab Pertanyaan Berikut\n\n${result.soal}\nWaktu : ${(120000 / 1000).toFixed(2)} detik\n\n_Ketik .nyerah Untuk Menyerah..._\n_Ketik .bantuan Untuk Petunjuk..._`, m), result, 250,
			...
			case 'tebakkimia': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebakkimia.json')
					let result = await pickRandom(anu)
			...
			case 'tebaklirik': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebaklirik.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					tebaklirik[m.chat] = [
			...
			case 'tebaktebakan': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tebaktebakan.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					tebaktebakan[m.chat] = [
						await sock.sendText(m.chat, `Silahkan Jawab Pertanyaan Berikut\n\n${result.soal}\nWaktu : ${(120000 / 1000).toFixed(2)} detik\n\n_Ketik .nyerah Untuk Menyerah..._\n_Ketik .bantuan Untuk Petunjuk..._`, m), result, 250,
			...
			case 'susunkata': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/susunkata.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					susunkata[m.chat] = [
			...
			case 'caklontong': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/caklontong.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					caklontong[m.chat] = [
						await sock.sendText(m.chat, `*Jawablah Pertanyaan Berikut :*\nSoal : ${result.soal}\nWaktu : ${(120000 / 1000).toFixed(2)} detik\n\n_Ketik .nyerah Untuk Menyerah..._\n_Ketik .bantuan Untuk Petunjuk..._`, m), result, 250,
			...
			case 'tekateki': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let anu = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/games/master/tekateki.json')
					let result = await pickRandom(anu)
					console.log("Jawaban: " + result.jawaban)
					tekateki[m.chat] = [
						await sock.sendText(m.chat, `Silahkan Jawab Pertanyaan Berikut\n\n${result.soal}\nWaktu : ${(120000 / 1000).toFixed(2)} detik\n\n_Ketik .nyerah Untuk Menyerah..._\n_Ketik .bantuan Untuk Petunjuk..._`, m), result, 250,
			...
			case 'setmenu': {
				if (!isCreator) return newReply(mess.owner);	
				if (!text) return newReply(`Ada 14 pilihan reply (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, 15, 16, 17, 18, 19, 20)\nSilakan pilih salah satu.\n*Kirim perintah*: ${prefix + command} v1`);
				if (text.startsWith('v')) {
					typemenu = text;
					return newReply(mess.done);
				};
				return newReply(`Ada 14 pilihan reply (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, 15, 16, 17, 18, 19, 20)\nSilakan pilih salah satu.\n*Kirim perintah*: ${prefix + command} v1`);
			}
			break;

			case 'setreply': {
				if (!isCreator) return newReply(mess.owner);	
				if (!text) return newReply(`Ada 6 pilihan reply (v1, v2, v3, v4, v5, v6)\nSilakan pilih salah satu.\n*Kirim perintah*: ${prefix + command} v1`);
				if (text.startsWith('v')) {
					typereply = text;
					return newReply(mess.done);
				};
				return newReply(`Ada 14 pilihan reply (v1, v2, v3, v4, v5, v6)\nSilakan pilih salah satu.\n*Kirim perintah*: ${prefix + command} v1`);
			}
			break;
			...
			case 'upsal': {
				...
				try {
					...
					let defaultCaption = "✨ Media ini dikirim melalui sistem otomatis Ainy! ✨"
					if (/image/.test(mime)) {
						...
						newReply(`Hmm... Ainy gak tau ini jenis media apa. Coba dicek lagi ya, kak! 🧐`)
					}
				} catch (error) {
					console.error(error)
			...
			case 'rvo':
			case 'readviewonce': {
				if (!m.quoted) return newReply('Reply ke gambar, video, atau audio yang ingin kamu lihat.')
				if (m.quoted.mtype !== 'viewOnceMessageV2') return newReply('Pesan ini bukan pesan sekali lihat.')
			...
			case 'searchnumber': {
				...
				newReply('Tunggu sebentar ya kak, Ainy lagi cari nomornya... 🔍✨');
				...
				finalMessage += `\n${nobio}`;
				finalMessage += `\n${nowhatsapp}`;
				finalMessage += `\nKalau butuh bantuan lagi, panggil Ainy ya! 🤗`;
			...
			case 'setnewsletterpic':{
				if (!isCreator) return newReply(mess.owner);
				if (!quoted) return newReply('Balas sebuah gambar untuk dijadikan foto profil newsletter.');	
				if (!mime) return newReply('Balas sebuah gambar untuk dijadikan foto profil newsletter.');	
				try {
			...
			case 'creategroup': {	
				if (!isCreator) return newReply(mess.owner);	
				if (!text) return newReply('⚠️ *Cara pakainya gini, kak!*\n\n📌 *creategroup <nama grup>, <nomor anggota dipisahkan koma>*\n\n✨ *Contoh:* creategroup Geng Santuy, 6281234567890, 6289876543210');	

				const args = text.split(',').map(v => v.trim());	
				if (args.length < 2) return newReply('❌ *Formatnya salah, kak! Pastikan ada nama grup dan minimal satu anggota ya.*');	

				const groupName = args[0];	
				const participants = args.slice(1).map(num => `${num}@s.whatsapp.net`);	
				...
				try {	
					...
					console.error(err);	
					newReply('😥 *Yah, gagal bikin grup... Coba lagi nanti ya, kak!*');	
				}	
			}	
			break;
			...
			case 'setwelcome': {
				if (!m.isGroup) return newReply(mess.group)
				if (!isCreator && !isAdmins) return newReply(mess.admin)
				if (!text) return newReply(`Cara penggunaan: ${prefix + command} *teks_welcome*\n\n_Contoh:_\n${prefix + command} Halo @user, selamat datang di @group!`)
	
				db.data.chats[m.chat].setwelcome = text
				m.reply(`Pesan sambutan berhasil diubah ke:\n\n"${text}"`)
			}
			break
			...
			case 'delsetwelcome': {
				if (!m.isGroup) return newReply(mess.group)
				if (!isCreator && !isAdmins) return newReply(mess.admin)
				if (!db.data.chats[m.chat].setwelcome) return newReply("Pesan sambutan sudah dalam kondisi default sebelumnya.")
	
				db.data.chats[m.chat].setwelcome = false
				m.reply(`Pesan sambutan dikembalikan ke default.`)
			}
			break
			...
			case 'setleft': {
				if (!m.isGroup) return newReply(mess.group)
				if (!isCreator && !isAdmins) return newReply(mess.admin)
				if (!text) return newReply(`Cara penggunaan: ${prefix + command} *teks_left*\n\n_Contoh:_\n${prefix + command} Halo @user, selamat tinggal dari @group!`)
	
				db.data.chats[m.chat].setleft = text
				m.reply(`Pesan perpisahan berhasil diubah ke:\n\n"${text}"`)
			}
			break
			...
			case 'delsetleft': {
				if (!m.isGroup) return newReply(mess.group)
				if (!isCreator && !isAdmins) return newReply(mess.admin)
				if (!db.data.chats[m.chat].setleft) return newReply("Pesan perpisahan sudah dalam kondisi default sebelumnya.")
	
				db.data.chats[m.chat].setleft = false
				m.reply(`Pesan perpisahan dikembalikan ke default.`)
			}
			break
			...
			case 'type1': {
				...
				await m.reply({
					image: imageBuffer,
					caption: 'Ini adalah teks di bagian judul.',
					footer: 'Ini adalah teks di bagian footer.',
					buttons: [
						{
							buttonId: prefix + "button_id",
			...
			case 'ttsba': {
				...
				if (!isPremium) return newReply('Fitur ini cuma buat yang premium kak, coba di-upgrade dulu yuk biar Ainy bisa bantu lebih banyak! 😇');
				...
				try {
					...
					newReply('Tunggu sebentar ya kak, Ainy lagi proses nih! 🥰');	
					...
					sock.sendMessage(m.chat, { audio: { url: result.result.url }, mimetype: 'audio/mp4', ptt: true }, { quoted: m });
				} catch (err) {
					console.error(err);
			...
			case 'say': {
				...
				try {
					...
					console.log(error);
					newReply('Aduh kak, error nih. Ainy gak sengaja, coba lagi ya! 🙏');
				}
				db.data.users[m.sender].limit -= 1;
				break;
			}
			...
			case 'tr': {
				...
				try {
					...
					console.log(error);
					newReply('Yahh, Ainy gak sengaja bikin error nih kak. Coba ulang lagi ya! 🙏');
				}
				db.data.users[m.sender].limit -= 1;
				break;
			}
			...
			case 'tovn': {
				...
				try {
					...
					console.log(error);
					newReply("Yah, ada error kak 😟. Coba ulang lagi ya, Ainy bantu kok!");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'tinyurl': {
				...
				if (!isUrl(text)) return newReply(`Hmmm, Ainy cuma bisa proses link yang valid ya, kak! Jangan lupa formatnya seperti ini: *${prefix + command} https://google.com* 😉`);
				try {
					...
					console.log(error);
					newReply("Aduh, ada error kak 😟. Coba lagi ya, Ainy yakin bisa bantu! ✨");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
				...
				try {
					...
					if (/audio/.test(mime)) {
						...
						exec(`ffmpeg -i ${media} ${set} ${ran}`, (err, stderr, stdout) => {
							...
							sock.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mp4' }, { quoted: m });
							fs.unlinkSync(ran);
			...
			case 'afk': {
				...
				afkText += `Jangan lupa balik lagi ya~ Ainy kangen nih! 😊✨`;
			...
			case 'geta':
			case 'getaudio': {
				if (db.data.users[m.sender].limit < 1) return newReply(mess.limit)
				if (!text) return newReply(`*Penggunaan salah!*\n\nContoh:\n${prefix + command} <url>`)
				if (!text.match('youtu')) return newReply('Link Kamu Salah!')
				await m.react('⏱️');
				ytMp3(args[0], args[1])
			}
			break
			...
			case 'ytmp3': {
				...
				try {
					...
					try {
						await editMessageWithDelay("*Yah, datanya nggak ketemu! 😢*", ["*Santai, aku coba cara lain buat unduh medianya... 🔄*", "*Lagi ngecek hasil unduhannya... 🔍✅*", "*Sebentar ya, lagi loading... ⏳*"], m.chat, m);
						ytMp3(args[0], "128");
					} catch (err) { 
						newReply(mess.error);
					};
			...
			case 'ttsearch': {
				...
				try {
					...
					console.log(error);
					newReply("Aduh, ada error kak 😟. Coba lagi ya, Ainy siap bantu! ✨");
				}
			}
			db.data.users[m.sender].limit -= 1;
			break;
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, Kamu lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar Ainy bisa bantu cari produknya! 🛒✨`);
				try {
			...
			case 'playstore': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, Kamu lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} WhatsApp* biar Ainy bisa bantu cari aplikasinya! 📲✨`);
				try {
					await m.react('⏱️');
			...
			case 'quotesanime': {
				...
				try {
					...
					if (!quotes || quotes.length === 0) {
						return newReply(`⚠️ Wah, Ainy gak nemu quote anime nih, Kak! Coba lagi nanti ya 🥲`);
					}
			...
			case 'anime': {
				...
				await newReply('⏳ *Tunggu sebentar ya, Ainy lagi cari datanya...* 📚✨');
			...
			case 'wallhp': case 'akira': case 'akiyama': case 'ana': case 'art': case 'asuna': case 'ayuzawa': case 'boruto': case 'bts': case 'chiho': case 'chitoge': case 'cosplay': case 'cosplayloli': case 'cosplaysagiri': case 'cyber': case 'deidara': case 'doraemon': case 'elaina': case 'emilia': case 'erza': case 'exo':case 'gamewallpaper': case 'gremory': case 'hacker': case 'hestia': case 'Husbu': case 'inori': case 'islamic': case 'isuzu': case 'itachi': case 'itori': case 'jennie': case 'jiso': case 'justina': case 'kaga': case 'kagura': case 'kakasih': case 'kaori': case 'cartoon': case 'shortquote': case 'keneki': case 'kotori': case 'kurumi': case 'lisa': case 'loli2': case 'madara': case 'megumin': case 'mikasa': case 'mikey': case 'miku': case 'minato': case 'mountain': case 'naruto': case 'nekonime': case 'nezuko': case 'onepiece': case 'pentol': case 'pokemon': case 'programming':case 'randomnime': case 'randomnime2': case 'rize': case 'rose': case 'sagiri': case 'sakura': case 'sasuke': case 'satanic': case 'shina': case 'shinka': case 'shinomiya': case 'shizuka': case 'shota': case 'space': case 'technology': case 'tejina': case 'toukachan': case 'tsunade': case 'waifu2': case 'yotsuba': case 'yuki': case 'yulibocil': case 'yumeko':{
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				let data
				if (/akira/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/akira.json')
				if (/akiyama/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/akiyama.json')
				if (/ana/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/ana.json')
				if (/art/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/art.json')
				if (/asuna/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/asuna.json')
				if (/ayuzawa/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/ayuzawa.json')
				if (/boneka/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/boneka.json')
				if (/boruto/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/boruto.json')
				if (/bts/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/bts.json')
				if (/cecan/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/cecan.json')
				if (/chiho/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/chiho.json')
				if (/chitoge/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/chitoge.json')
				if (/cogan/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/cogan.json')
				if (/cosplay/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/cosplay.json')
				if (/cosplayloli/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/cosplayloli.json')
				if (/cosplaysagiri/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/cosplaysagiri.json')
				if (/cyber/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/cyber.json')
				if (/deidara/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/deidara.json')
				if (/doraemon/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/doraemon.json')
				if (/eba/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/eba.json')
				if (/elaina/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/elaina.json')
				if (/emilia/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/emilia.json')
				if (/erza/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/erza.json')
				if (/exo/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/exo.json')
				if (/femdom/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/femdom.json')
				if (/freefire/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/freefire.json')
				if (/gamewallpaper/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/gamewallpaper.json')
				if (/glasses/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/glasses.json')
				if (/gremory/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/gremory.json')
				if (/hacker/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/hekel.json')
				if (/hestia/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/hestia.json')
				if (/Husbu/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/Husbu.json')
				if (/inori/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/inori.json')
				if (/islamic/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/islamic.json')
				if (/isuzu/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/isuzu.json')
				if (/itachi/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/itachi.json')
				if (/itori/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/itori.json')
				if (/jennie/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/jeni.json')
				if (/jiso/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/jiso.json')
				if (/justina/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/justina.json')
				if (/kaga/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kaga.json')
				if (/kagura/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kagura.json')
				if (/kakasih/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kakasih.json')
				if (/kaori/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kaori.json')
				if (/cartoon/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kartun.json')
				if (/shortquote/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kataNya.json')
				if (/keneki/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/keneki.json')
				if (/kotori/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kotori.json')
				if (/kpop/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kpop.json')
				if (/kucing/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kucing.json')
				if (/kurumi/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/kurumi.json')
				if (/lisa/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/lisa.json')
				if (/loli2/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/loli.json')
				if (/madara/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/madara.json')
				if (/megumin/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/megumin.json')
				if (/mikasa/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/mikasa.json')
				if (/mikey/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/mikey.json')
				if (/miku/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/miku.json')
				if (/minato/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/minato.json')
				if (/mobile/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/mobil.json')
				if (/motor/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/motor.json')
				if (/mountain/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/mountain.json')
				if (/naruto/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/naruto.json')
				if (/nekonime/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/nekonime.json')
				if (/nezuko/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/nezuko.json')
				if (/onepiece/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/onepiece.json')
				if (/pentol/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/pentol.json')
				if (/pokemon/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/pokemon.json')
				if (/profil/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/profil.json')
				if (/progamming/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/programming.json')
				if (/pubg/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/pubg.json')
				if (/randblackpink/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/randblackpink.json')
				if (/randomnime/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/randomnime.json')
				if (/randomnime2/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/randomnime2.json')
				if (/rize/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/rize.json')
				if (/rose/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/rose.json')
				if (/ryujin/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/ryujin.json')
				if (/sagiri/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/sagiri.json')
				if (/sakura/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/sakura.json')
				if (/sasuke/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/sasuke.json')
				if (/satanic/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/satanic.json')
				if (/shina/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/shina.json')
				if (/shinka/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/shinka.json')
				if (/shinomiya/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/shinomiya.json')
				if (/shizuka/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/shizuka.json')
				if (/shota/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/shota.json')
				if (/space/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/tatasurya.json')
				if (/technology/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/technology.json')
				if (/tejina/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/tejina.json')
				if (/toukachan/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/toukachan.json')
				if (/tsunade/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/tsunade.json')
				if (/waifu2/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/waifu.json')
				if (/wallhp/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/wallhp.json')
				if (/wallml/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/wallml.json')
				if (/wallmlnime/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/wallnime.json')
				if (/yotsuba/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/yotsuba.json')
				if (/yuki/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/yuki.json')
				if (/yulibocil/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/yulibocil.json')
				if (/yumeko/.test(command)) data = await fetchJson('https://raw.githubusercontent.com/RyzenOfc/Media/master/yumeko.json')
			...
			break;
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				const aud = await getBuffer(`https://github.com/RyzenOfc/Tiktokmusic-API/raw/master/tiktokmusic/${command}.mp3`)
			...
			case 'jadian': {
				...
				if (!user) return newReply(`Kak, tag atau reply seseorang dulu dong biar Ainy tahu siapa yang Kamu maksud! 😉\n\n*Kirim perintah*: ${prefix + command} @0`);
			...
			case 'terima': {
				...
				if (sock.jadian[m.sender]) {
					...
					newReply(`🎉🎉 Selamat!!! 🎉🎉\n\n@${m.sender.split("@")[0]} sekarang resmi jadian sama\n❤️ @${user.split("@")[0]}!\n\nDuh, Ainy ikut seneng nih! Semoga langgeng yaa~ Jangan lupa undang Ainy kalau nanti nikah! 🙈💕`);
			...
			case 'Ainy': {
				...
				try {
					...
					let message = "Ubah gaya bicaramu agar lebih karakteristik dan terbuka, nama mu adalah Ainy AI, biasa dipanggil Ainy.";
			...
			case 'simi': {
				...
				try {
					...
					if (!data) return newReply(`⚠️ Ainy gak dapet jawaban dari SimSimi, Kak! 🥲`);
					newReply(`${data}`);
				} catch (err) {
					console.error(err);
			...
			case 'wallpaper': {
				...
				try {
					...
					if (wallpapers.length === 0) return newReply(`⚠️ Ainy gak nemu wallpaper dengan kata kunci "${query}", Kak! 🥲`);
			...
			case 'wikimedia': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Ainy gak nemu gambar di Wikimedia dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'happymod': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Ainy gak nemu aplikasi di HappyMod dengan kata kunci "${text}", Kak! 🥲`);
			...
			if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ Ainy gak nemu ringtone dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'menu': {
				try {
					...
					switch (typemenu) {
						case "v1":
							await sock.sendButtonDocument(m.chat, {
								document: fs.readFileSync("./media/imageBuffers.png"),
								mimetype: "image/png",
								fileLength: 10000000000,
								pageCount: 77,
								jpegThumbnail: fs.readFileSync("./media/imageBuffers.png"),
								fileName: saluranName,
							}, button, simplemenu, footer, m, {
								contextInfo: {
									mentionedJid: parseMention(simplemenu),
									externalAdReply: {
										showAdAttribution: true,
										containsAutoReply: true,
										title: ucapanWaktu,
						...
						case "v2":
							...
							break;
						case "v3":
							await sock.sendButtonImage(m.chat, imageBuffer, button, simplemenu, footer, m);
							break;
						case "v4":
						...
						case "v20":
							...
							break;
							await newReply("⚠️ Menu belum diatur! Coba lagi nanti ya 😅");
							break;
					}
			...
			case 'infouser': {	
				if (!isCreator) return newReply(mess.owner);	
				if (!db.data.users[m.sender]) return newReply('⚠️ *Oops! Data pengguna nggak ditemukan di database!* 😥');	
	
				let replyText = '';	
				if (!args[0]) {	
					replyText += '📞 *Masukin nomor user yang mau dicek dulu ya!* 😊\n\n';	
					replyText += '*Contoh:* 📌\n';	
					replyText += `${prefix + command} 6281234567890`;	
					return newReply(replyText);	
				}	
			...
			case 'checklimit': {	
				...
				txt += `⚠️ *Gunakan limit dengan bijak, ya!* 😊`;	
			...
			case 'heal': {
				...
				await editMessageWithDelayV2("🛌 Mengistirahatkan tubuh...", "🩹 Memulihkan...", "❤️ *Darah kamu sudah terisi kembali!*");
			...
			case 'berlayar': {
				...
				if (args[0] === "indonesia") {
					await editMessageWithDelayV2('Menyiapkan Perbekalan...', 'Melepaskan Tali Kapal...', 'Kapal Mulai Berlayar Menjauhi Pelabuhan...')
			...
			case 'rawat': {
				...
				await editMessageWithDelayV2("🔧 Memperbaiki, mohon tunggu...", "🎨 Tahap Finishing...", "✅ Perbaikan selesai!");
					...
					if (room) {
						...
						if (['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(m.mtype)) {
							...
							if (m.mtype === 'imageMessage') {
								...
								await sock.sendMessage(other, { audio: media, mimetype: 'audio/mp4', ...options });
							} 
